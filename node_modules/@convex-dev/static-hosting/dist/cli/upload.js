...
import { componentNameCandidates, isLegacyAutoDetected, legacyComponentNameWarning, } from "./componentName.js";
// MIME type mapping
const MIME_TYPES = {
 ...
 */
async function fetchUrls(requested) {
    const candidates = componentNameCandidates(requested);
    for (const name of candidates) {
        try {
            const out = await convexRunAsync(name, "lib:getUrls", {}, { quiet: true });
            const urls = JSON.parse(out);
...
async function cleanUpPendingCdnBlobs(componentName, cdnDeleteFunction) {
    if (!cdnDeleteFunction)
        return 0;
    let cleaned = 0;
    for (let page = 0; page < MAX_MAINTENANCE_PAGES; page++) {
        const output = await convexRunAsync(componentName, "lib:listPendingBlobCleanup", { limit: 250 });
        const blobIds = JSON.parse(output);
    ...
    console.warn("Warning: CDN cleanup reached its per-run maintenance limit; a later upload will continue it.");
    return cleaned;
}
async function cleanUpAbandonedUploads(componentName, cdnDeleteFunction) {
    let discarded = 0;
    let deletedFiles = 0;
    let queuedBlobIds = 0;
    try {
        ...
        for (let page = 0; page < MAX_MAINTENANCE_PAGES; page++) {
            const output = await convexRunAsync(componentName, "lib:cleanupAbandonedStaging", { limit: MAINTENANCE_PAGE_SIZE });
        ...
        if (!stagingDrained) {
            console.warn("Warning: Staging cleanup reached its per-run maintenance limit; a later upload will continue it.");
        }
        deletedFiles += await cleanUpPendingStorage(componentName);
        deletedFiles += await cleanUpUnreferencedStorage(componentName, undefined, "desc");
        deletedFiles += await cleanUpUnreferencedStorage(componentName, undefined, "asc");
        if (discarded > 0 || deletedFiles > 0 || queuedBlobIds > 0) {
            console.log(`Recovered ${discarded} abandoned manifest record(s), removed ${deletedFiles} old unreferenced file(s), and queued ${queuedBlobIds} CDN blob(s) for cleanup.`);
        }
        if (cdnDeleteFunction) {
            const cleaned = await cleanUpPendingCdnBlobs(componentName, cdnDeleteFunction);
            if (cleaned > 0) {
        ...
        console.warn("Warning: Could not finish abandoned-upload maintenance: " +
            errorMessage(cleanupError));
    }
}
async function cleanUpUnreferencedStorage(componentName, before, order = "asc") {
    let deleted = 0;
    for (let page = 0; page < MAX_MAINTENANCE_PAGES; page++) {
        const output = await convexRunAsync(componentName, "lib:cleanupUnreferencedStorage", {
    ...
    console.warn("Warning: Orphan storage cleanup reached its per-run maintenance limit; a later upload will continue it.");
    return deleted;
}
async function cleanUpPendingStorage(componentName) {
    let deleted = 0;
    for (let page = 0; page < MAX_MAINTENANCE_PAGES; page++) {
        const output = await convexRunAsync(componentName, "lib:cleanupPendingStorage", { limit: MAINTENANCE_PAGE_SIZE });
        const result = JSON.parse(output);
        if (typeof result.deleted !== "number" ||
            typeof result.needsAnotherPass !== "boolean") {
            throw new Error("Component returned invalid pending-storage cleanup data");
        }
        deleted += result.deleted;
        if (!result.needsAnotherPass)
            return deleted;
...
async function cleanUpFailedUpload(componentName, deploymentId, uploaded, cdnDeleteFunction, publishAttempted) {
    let discardSucceeded = false;
    let discarded = 0;
    try {
        const output = await convexRunAsync(componentName, "lib:discardStagedDeployment", { deploymentId });
        const result = JSON.parse(output);
        if (typeof result.discarded !== "number") {
        ...
        console.warn("Warning: Could not discard the staged manifest: " +
            errorMessage(cleanupError));
    }
    ...
    if (uploaded.storageIds.length > 0 && canCleanComponentFiles) {
        try {
            ...
            const chunks = chunkBySerializedArgument(uploaded.storageIds, (storageIds) => ({ storageIds }));
            for (const storageIds of chunks) {
                const output = await convexRunAsync(componentName, "lib:deleteUploadedFiles", { storageIds });
                const result = JSON.parse(output);
                if (typeof result.deleted === "number")
                    deleted += result.deleted;
            ...
            if (stillReferenced > 0) {
                console.warn(`Kept ${stillReferenced} file(s) because the live deployment references them. The publish may have completed even though its response was lost.`);
            }
        }
        catch (cleanupError) {
            console.warn("Warning: Could not remove component files from the failed upload: " +
                errorMessage(cleanupError));
        }
    }
    else if (uploaded.storageIds.length > 0) {
        console.warn(`${uploaded.storageIds.length} component file(s) were left in place because the CLI could not cancel or confirm the publish.`);
    }
    ...
    const canDeleteCdnBlobs = !publishAttempted || (discardSucceeded && discarded > 0);
    if (uploaded.blobIds.length > 0 && canDeleteCdnBlobs) {
        try {
            ...
            if (cdnDeleteFunction) {
                ...
                console.warn(`${uploaded.blobIds.length} CDN blob(s) from the failed upload were queued for cleanup. Pass --cdn-delete-function on a later upload to delete them.`);
            }
        }
        catch (cleanupError) {
            console.warn("Warning: Could not finish queuing or deleting CDN blobs from the failed upload: " +
                errorMessage(cleanupError));
        }
    }
    else if (uploaded.blobIds.length > 0 && !canDeleteCdnBlobs) {
        console.warn(`${uploaded.blobIds.length} CDN blob(s) were left in place because the CLI could not determine whether the publish completed.`);
    }
}
async function uploadWithConcurrency(files, componentName, deploymentId, useCdn, cdnUploadBase, concurrency, spaFallback, cdnDeleteFunction) {
    const total = files.length;
    const uploaded = { storageIds: [], blobIds: [] };
    ...
    try {
        if (storageFiles.length > 0) {
            ...
            const uploadUrls = [];
            for (let offset = 0; offset < storageFiles.length; offset += UPLOAD_URL_BATCH_SIZE) {
                const count = Math.min(UPLOAD_URL_BATCH_SIZE, storageFiles.length - offset);
                const urlsOutput = await convexRunAsync(componentName, "lib:generateUploadUrls", { count });
                const batch = JSON.parse(urlsOutput);
            ...
            await forEachWithConcurrency(storageFiles, concurrency, async (file, idx) => {
                ...
                if (!response.ok) {
                    throw new Error("Storage upload failed for " +
                        file.path +
                        ": " +
                        response.status +
                        " " +
                ...
                if (typeof storageId !== "string") {
                    throw new Error("Storage upload returned no storageId for " + file.path);
                }
        ...
        for (let index = 0; index < manifestChunks.length; index++) {
            console.log(`  Staging manifest chunk ${index + 1}/${manifestChunks.length}...`);
        ...
        const output = await convexRunAsync(componentName, "lib:publishDeployment", {
        ...
        if (publishAttempted) {
            try {
                const output = await convexRunAsync(componentName, "lib:getCurrentDeployment");
                const current = JSON.parse(output);
                if (current?.currentDeploymentId === deploymentId) {
                    console.warn("The publish completed, but the CLI could not read a valid response. The new deployment is live and its files were kept.");
                    return {
                        deleted: 0,
                        pendingBlobCleanup: typeof current.pendingBlobCleanupCount === "number"
                ...
                console.warn("Warning: Could not determine whether the publish completed: " +
                    errorMessage(reconcileError));
            }
        }
        await cleanUpFailedUpload(componentName, deploymentId, uploaded, cdnDeleteFunction, publishAttempted);
        throw error;
    }
}
...
function estimateManifestBytes(files, deploymentId) {
    ...
    return Buffer.byteLength(JSON.stringify(files.map((file) => ({
        path: file.path,
...
async function main() {
    ...
    const { urls: { siteUrl: componentSiteUrl, cloudUrl: convexUrl }, componentName: resolvedComponentName, } = await fetchUrls(args.component);
    ...
    if (files.length > MAX_ASSETS_PER_DEPLOYMENT) {
        console.error(`Error: dist directory contains ${files.length} files; the supported maximum is ${MAX_ASSETS_PER_DEPLOYMENT}.`);
        console.error("Reduce the number of emitted files so publication stays within Convex transaction limits.");
        process.exit(1);
    }
    ...
    if (estimatedManifestBytes > MAX_MANIFEST_SERIALIZED_BYTES) {
        console.error(`Error: asset manifest metadata is approximately ${estimatedManifestBytes} bytes; the supported maximum is ${MAX_MANIFEST_SERIALIZED_BYTES}.`);
        console.error("Shorten emitted asset paths or reduce the number of files so publication stays within Convex transaction byte limits.");
        process.exit(1);
    }
    ...
    console.log("");
    const publishResult = await uploadWithConcurrency(files, componentName, deploymentId, useCdn, cdnUploadBase, args.concurrency, args.spaFallback, args.cdnDeleteFunction);
    console.log("");
    ...
    try {
        ...
        console.warn("Warning: The new deployment is live, but old component storage could not be fully removed: " +
            errorMessage(cleanupError));
    }
    if (deletedCount > 0) {
    ...
    if (args.cdnDeleteFunction) {
        try {
            const cleaned = await cleanUpPendingCdnBlobs(componentName, args.cdnDeleteFunction);
            if (cleaned > 0) {
                console.log(`Cleaned up ${cleaned} old CDN blob(s) from previous deployments`);
            }
        }
        catch (error) {
            console.warn("Warning: Could not delete old CDN blobs via " +
        ...
        console.log(`${publishResult.pendingBlobCleanup} CDN blob(s) are pending cleanup. Pass --cdn-delete-function to delete them.`);
    }
    console.log("");
