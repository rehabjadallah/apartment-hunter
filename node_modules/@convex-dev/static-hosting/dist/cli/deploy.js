...
import { runConvex, spawnConvex, spawnShell, spawnStaticHostingCli, } from "./commands.js";
import { buildDeployUploadArgs, parseDeployArgs, } from "./args.js";
import { buildEnvironment, buildEnvironmentChanged, } from "./deployEnvironment.js";
 ...
 */
function tryFetchUrls(componentName) {
    try {
        const out = runConvex([
            "run",
            "--component",
 ...
 * retry).
 */
function tryResolveComponent(requested) {
...
function resolveComponentOrExit(requested) {
    const resolved = tryResolveComponent(requested);
    if (!resolved) {
        const names = componentNameCandidates(requested)
            .map((name) => `"${name}"`)
            .join(" or ");
        console.error(`Could not reach component ${names}. Deploy the Convex backend first (npx convex deploy) and ensure --component matches the name in convex.config.ts.`);
        process.exit(1);
    }
    return resolved;
}
 ...
 */
async function uploadToConvexStorage(args) {
    console.log("");
    console.log(args.cdn
...
function runFrontendBuild(args, urls) {
...
async function main() {
    ...
    if (!urls && !args.skipConvex) {
        console.log("   Component not yet deployed. Will fetch URLs after deploying backend.");
    }
    else if (urls) {
    ...
    if (shouldDeployBackend && !backendAlreadyDeployed) {
        ...
        if (!args.skipBuild) {
            if (!urls || buildEnvironmentChanged(urls, deployedUrls)) {
                console.log("");
                console.log("   Static hosting URL changed after backend deployment. Rebuilding with the new environment...");
                try {
