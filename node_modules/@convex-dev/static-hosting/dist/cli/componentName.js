 ...
 */
export function componentNameCandidates(requested) {
    if (requested !== DEFAULT_COMPONENT_NAME)
        return [requested];
 ...
 */
export function isLegacyAutoDetected(requested, resolved) {
    return (requested === DEFAULT_COMPONENT_NAME && resolved === LEGACY_COMPONENT_NAME);
}
/** Warning shown once when a command resolves to the legacy instance name. */
export function legacyComponentNameWarning(name) {
    return (`⚠️  Using the legacy component name "${name}" (the 0.1.x default; the ` +
