...
async function resolveAssetDocument(ctx, path, spaFallbackOverride) {
...
async function deleteStorageFile(ctx, storageId) {
...
async function adjustPendingBlobCleanupCount(ctx, delta) {
...
export const getCurrentDeployment = query({
    ...
    handler: async (ctx) => {
        ...
        return {
            ...info,
            pendingBlobCleanupCount: state?.pendingBlobCleanupCount ?? info.pendingBlobCleanupCount,
        };
    },
});
...
export const resolveAssetForHttp = query({
    ...
    handler: async (ctx, { path, spaFallback }) => {
        ...
        // here instead.
        const appStorageId = asset.storageId && !storageUrl && !asset.blobId
            ? asset.storageId
            : undefined;
        return {
...
export const listPendingBlobCleanup = internalQuery({
    ...
    handler: async (ctx, { limit }) => {
        return (await ctx.db.query("pendingBlobCleanup").take(Math.min(limit ?? 250, 500))).map((entry) => entry.blobId);
    },
});
...
export const queueBlobCleanup = internalMutation({
    ...
    handler: async (ctx, { blobIds }) => {
        ...
        if (queued > 0)
            await adjustPendingBlobCleanupCount(ctx, queued);
        return { queued };
    },
});
export const cleanupPendingStorage = internalMutation({
    ...
    handler: async (ctx, { limit }) => {
        const batchLimit = Math.min(limit ?? MAINTENANCE_PAGE_LIMIT, MAINTENANCE_PAGE_LIMIT);
        const entries = await ctx.db
...
export const deleteUploadedFiles = internalMutation({
    ...
    handler: async (ctx, { storageIds }) => {
        ...
        const referencedStorageIds = new Set(liveAssets.flatMap((asset) => asset.storageId === undefined ? [] : [asset.storageId]));
        let deleted = 0;
...
function assertAssetLocation({ path, storageId, blobId, }) {
    if ((storageId === undefined) === (blobId === undefined)) {
        throw new Error("Asset " + path + " must have exactly one of storageId or blobId");
    }
}
function assertAssetManifest(assets, deploymentId, requireIndex = false) {
    ...
    if (requireIndex && assets.length > MAX_ASSETS_PER_DEPLOYMENT) {
        throw new Error(`A static asset manifest can contain at most ${MAX_ASSETS_PER_DEPLOYMENT} files`);
    }
    const paths = new Set();
...
export const cleanupUnreferencedStorage = internalMutation({
    ...
    handler: async (ctx, { before, limit, order }) => {
        ...
        const liveStorageIds = new Set(liveAssets.flatMap((asset) => asset.storageId === undefined ? [] : [asset.storageId]));
        const stagedLookupBudget = Math.max(0, 3900 - liveAssets.length - files.length);
        const deleteLimit = Math.min(limit ?? MAINTENANCE_PAGE_LIMIT, MAINTENANCE_PAGE_LIMIT);
        let deleted = 0;
        let scanned = 0;
        let stagedLookups = 0;
        ...
        for (const file of files) {
            ...
            scanned++;
            if (liveStorageIds.has(file._id))
                continue;
        ...
        return {
            scanned,
            deleted,
            needsAnotherPass: deleted === deleteLimit ||
                stoppedForReadBudget ||
                (deleted > 0 && files.length === STORAGE_SCAN_LIMIT),
        };
    },
});
async function setCurrentDeployment(ctx, currentDeploymentId, spaFallback, pendingBlobCleanupCount) {
...
export const publishDeployment = internalMutation({
    ...
    handler: async (ctx, { currentDeploymentId, expectedAssetCount, spaFallback }) => {
        if (!Number.isInteger(expectedAssetCount) ||
            expectedAssetCount < 1 ||
            expectedAssetCount > MAX_ASSETS_PER_DEPLOYMENT) {
            throw new Error(`expectedAssetCount must be an integer from 1 to ${MAX_ASSETS_PER_DEPLOYMENT}`);
        }
        const stagedAssets = await ctx.db
            .query("stagedAssets")
            .withIndex("by_deploymentId", (q) => q.eq("deploymentId", currentDeploymentId))
            .take(expectedAssetCount + 1);
        const assets = stagedAssets.map(({ path, storageId, blobId, contentType, deploymentId }) => ({
            path,
            storageId,
            blobId,
            contentType,
        ...
        if (assets.length !== expectedAssetCount) {
            throw new Error(`Staged manifest has ${assets.length} files; expected ${expectedAssetCount}`);
        }
        assertAssetManifest(assets, currentDeploymentId, true);
        const manifestBytes = new TextEncoder().encode(JSON.stringify(assets)).byteLength;
        if (manifestBytes > MAX_MANIFEST_SERIALIZED_BYTES) {
            throw new Error(`The staged manifest is ${manifestBytes} bytes, above the safe limit of ${MAX_MANIFEST_SERIALIZED_BYTES} bytes. Shorten asset paths or reduce the number of files.`);
        }
        ...
        if (assets.length + oldAssets.length > MAX_PUBLISH_MANIFEST_READS) {
            throw new Error(`The old and new manifests contain more than ${MAX_PUBLISH_MANIFEST_READS} rows combined. Reduce the new build or clean up the legacy manifest before migrating.`);
        }
        const blobIds = [];
        ...
        const plannedDocumentWrites = oldAssets.length +
            storageIds.size +
            blobIds.length +
            assets.length +
            stagedAssets.length +
        ...
        if (plannedDocumentWrites > MAX_PUBLISH_DOCUMENT_WRITES) {
            throw new Error(`Publishing this manifest would require ${plannedDocumentWrites} document writes, above the safe limit of ${MAX_PUBLISH_DOCUMENT_WRITES}. Reduce the number of files in the deployment.`);
        }
        for (const asset of oldAssets) {
        ...
        const pendingBlobCleanup = await adjustPendingBlobCleanupCount(ctx, blobIds.length);
        await setCurrentDeployment(ctx, currentDeploymentId, spaFallback ?? true, pendingBlobCleanup);
...
export const commitDeployment = internalMutation({
    ...
    handler: async (ctx, args) => {
        ...
        await setCurrentDeployment(ctx, args.currentDeploymentId, args.spaFallback ?? true);
