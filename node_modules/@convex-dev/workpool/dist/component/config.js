...
import { vConfig, DEFAULT_MAX_PARALLELISM } from "./shared.js";
...
export function validateConfig(config) {
    if (config.maxParallelism !== undefined) {
        if (config.maxParallelism > MAX_POSSIBLE_PARALLELISM) {
            ...
            createLogger(config.logLevel ?? DEFAULT_LOG_LEVEL).warn(`maxParallelism should be <= ${MAX_PARALLELISM_SOFT_LIMIT}, but is set to ${config.maxParallelism}. This will be an error in a future version.`);
        }
...
export async function getOrUpdateGlobals(ctx, config) {
    const { globals } = await _getOrUpdateGlobals(ctx, config);
    return globals;
}
async function _getOrUpdateGlobals(ctx, config) {
    if (config) {
