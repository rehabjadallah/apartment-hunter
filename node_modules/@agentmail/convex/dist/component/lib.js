...
export const enqueueSend = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        await sendPool.enqueueAction(ctx, internal.lib.performSend, { outboundId: id }, {
            retry: {
...
const vSendResult = v.union(v.null(), v.object({
...
export const onSendComplete = sendPool.defineOnComplete({
    ...
    handler: async (ctx, args) => {
        ...
        if (args.result.kind === "success") {
            ...
            if (value === null)
                return; // permanent error path already wrote failure
...
export const handleEvent = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        if (args.config.onEvent) {
            const handle = args.config.onEvent.fnHandle;
...
async function applyEventToOutbound(ctx, messageId, event) {
    ...
    if (isTerminalStatus(outbound.status))
        return;
