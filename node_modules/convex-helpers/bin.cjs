...
function getFunctionSpec(prod, filePath) {
  ...
  if (filePath && !import_fs.default.existsSync(filePath)) {
    console.error(import_chalk.default.red(`File ${filePath} not found`));
    process.exit(1);
  }
  if (filePath) {
...
var openApiSpec = new Command("open-api-spec").summary("Generate an OpenAPI spec from a Convex function definition.").addOption(
  ...
  const baseOutputPath = options.outputFile ?? `convex-spec-${Date.now().valueOf()}`;
  const outputPath = baseOutputPath.endsWith(".yaml") || baseOutputPath.endsWith(".yml") ? baseOutputPath : baseOutputPath + ".yaml";
  try {
  ...
  console.log(import_chalk2.default.green("Wrote OpenAPI spec to " + outputPath));
});
...
var tsApiSpec = new Command("ts-api-spec").summary(
  ...
  const content = getFunctionSpec(options.prod, options.inputFile);
  const baseOutputPath = options.outputFile ?? `convexApi${Date.now().valueOf()}`;
  const outputPath = baseOutputPath.endsWith(".ts") ? baseOutputPath : baseOutputPath + ".ts";
  try {
  ...
  console.log(import_chalk3.default.green("Wrote JavaScript API spec to " + outputPath));
});
