...
function toPublicCrawl(crawl) {
    const { token: _token, onComplete: _onComplete, nextUrl: _nextUrl, pollAttempt: _pollAttempt, ...rest } = crawl;
    return rest;
}
 ...
 */
function resolveId(ctx, crawlId) {
    return ctx.db.normalizeId("crawls", crawlId);
}
function requireId(ctx, crawlId) {
...
function nextDelayMs(mode, attempt) {
...
function fitString(value, maxBytes) {
    if (maxBytes <= 2)
        return "";
...
function fitArray(values, maxBytes) {
    const kept = [];
    let used = 2; // []
...
function essentialMetadata(metadata) {
 ...
 */
export function clampContent(fields) {
    const clamped = { url: fields.url, truncated: fields.truncated };
    ...
    for (const key of [
        ...
        if (key === "markdown" || key === "summary" || key === "html" || key === "rawHtml") {
            const cut = fitString(value, allowance);
            if (cut.length > 0) {
                clamped[key] = cut;
            ...
            const kept = fitArray(value, allowance);
            if (kept.length > 0) {
                clamped.links = kept;
...
export function toPageFields(document, storeContent) {
    const metadata = document?.metadata ?? {};
    const url = metadata.url ?? metadata.sourceURL ?? document?.url;
    if (typeof url !== "string" || url.length === 0)
        return null;
    const fields = { url, metadata, truncated: false };
    ...
    if (typeof document.markdown === "string")
        fields.markdown = document.markdown;
    if (typeof document.html === "string")
        fields.html = document.html;
    if (typeof document.rawHtml === "string")
        fields.rawHtml = document.rawHtml;
    if (typeof document.summary === "string")
        fields.summary = document.summary;
    if (typeof document.screenshot === "string")
        fields.screenshot = document.screenshot;
    if (Array.isArray(document.links))
        fields.links = document.links.filter((link) => typeof link === "string");
    if (document.json !== undefined)
        fields.json = document.json;
    ...
    handler: async (ctx, args) => {
        ...
        if (mode === "webhook" && !siteUrl) {
            throw new ConvexError({
                ...
                message: "Webhook mode needs CONVEX_SITE_URL. Mount the component's HTTP routes (`app.use(firecrawl, { httpPrefix: \"/firecrawl/\" })`) or pass mode: \"poll\".",
            });
        }
        ...
        const crawlId = await ctx.runMutation(internal.crawl.createCrawl, {
            url: args.url,
            mode,
            token,
            storeContent,
        ...
        try {
            ...
            await ctx.scheduler.runAfter(nextDelayMs(mode, 0), internal.crawl.poll, { crawlId });
...
export const listCrawls = query({
    ...
    handler: async (ctx, args) => {
        ...
        const crawls = args.status !== undefined
            ? await ctx.db
                .query("crawls")
...
export const listPages = query({
    ...
    handler: async (ctx, args) => await paginator(ctx.db, schema)
        .query("pages")
        .withIndex("crawlId_url", (q) => q.eq("crawlId", requireId(ctx, args.crawlId)))
...
export const cancel = action({
    ...
    returns: v.null(),
    handler: async (ctx, args) => {
        const crawl = await ctx.runQuery(internal.crawl.getByIdString, { crawlId: args.crawlId });
        if (!crawl) {
            throw new ConvexError({
                code: "firecrawl_unknown_crawl",
...
export const deleteCrawl = mutation({
    ...
    handler: async (ctx, args) => {
        const id = resolveId(ctx, args.crawlId);
        if (id)
            await deletePages(ctx, id);
        return null;
    },
});
async function deletePages(ctx, crawlId) {
    ...
    if (crawl)
        await ctx.db.delete("crawls", crawlId);
}
...
export const createCrawl = internalMutation({
    ...
    returns: v.id("crawls"),
    handler: async (ctx, args) => await ctx.db.insert("crawls", {
        ...args,
        status: "scraping",
        pageCount: 0,
        finalized: false,
 ...
 */
async function ingestDocuments(ctx, crawlId, documents) {
    const crawl = await ctx.db.get("crawls", crawlId);
    if (!crawl)
        return 0;
    let added = 0;
    for (const document of documents) {
        ...
        const existing = await ctx.db
            .query("pages")
            .withIndex("crawlId_url", (q) => q.eq("crawlId", crawlId).eq("url", fields.url))
            .first();
        if (existing) {
...
export const ingestPages = internalMutation({
    ...
    returns: v.number(),
    handler: async (ctx, args) => await ingestDocuments(ctx, args.crawlId, args.documents),
});
/**
...
export const advance = internalMutation({
    ...
    handler: async (ctx, args) => {
        ...
        if (pollAttempt >= MAX_POLL_ATTEMPTS) {
            ...
            await finalizeCrawl(ctx, args.crawlId, "failed", `Stopped tracking after ${MAX_POLL_ATTEMPTS} status checks. The crawl may still be running on Firecrawl — call resume({ crawlId }) to pick tracking back up.`);
            return null;
        }
        await ctx.scheduler.runAfter(nextDelayMs(crawl.mode, pollAttempt), internal.crawl.poll, { crawlId: args.crawlId });
        return null;
    },
});
...
export const poll = internalAction({
    ...
    returns: v.null(),
    handler: async (ctx, args) => {
        const crawl = await ctx.runQuery(internal.crawl.getInternal, { crawlId: args.crawlId });
        if (!crawl || crawl.finalized || !crawl.jobId)
            return null;
        let body;
        try {
        ...
        for (let i = 0; i < documents.length; i += INGEST_BATCH) {
            ...
            try {
                ...
                console.error(`firecrawl: could not store ${batch.length} page(s) of crawl ${args.crawlId}: ${messageOf(error)}`);
            }
        }
...
export const handleWebhook = internalMutation({
    ...
    handler: async (ctx, args) => {
        ...
        if (event === "completed") {
            ...
            await finalizeCrawl(ctx, crawl._id, "failed", args.error ?? "Firecrawl reported the crawl failed.");
        }
        else {
...
async function finalizeCrawl(ctx, crawlId, status, error) {
    const crawl = await ctx.db.get("crawls", crawlId);
    if (!crawl || crawl.finalized)
        return;
    await ctx.db.patch("crawls", crawlId, {
