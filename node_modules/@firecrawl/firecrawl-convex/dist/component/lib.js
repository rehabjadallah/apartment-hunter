...
import { mapOptionsValidator, scrapeOptionsValidator, searchOptionsValidator, } from "./validators.js";
/**
 ...
 */
export function withExtra(options) {
    if (!options)
        return {};
    const { extra, scrapeOptions, ...rest } = options;
    const flattened = { ...rest, ...(extra ?? {}) };
    if (scrapeOptions)
        flattened.scrapeOptions = withExtra(scrapeOptions);
    return flattened;
}
...
export const map = action({
    ...
    handler: async (_ctx, args) => {
        ...
        return {
            id: body.id,
            links: links.map((link) => typeof link === "string" ? { url: link } : link),
        };
    },
});
